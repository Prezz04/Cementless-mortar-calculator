...
def load_prediction_engine():
    try:
        ...
        # Ambil booster murni untuk menghindari error versi sklearn di server
...
# ==========================================
# 3. INTERFACE INPUT (Kembali Menggunakan Rasio Desimal Asli Anda)
# ==========================================
if xgb_engine is not None:
    st.write("### Input Mix Design Parameters (Rasio Desimal):")
    col1, col2, col3 = st.columns(3)
    
    with col1:
        ggbs = st.number_input("GGBS (ratio)", min_value=0.0, max_value=1.0, value=0.60, step=0.05)
        sf = st.number_input("Silica Fume (ratio)", min_value=0.0, max_value=1.0, value=0.10, step=0.01)
        fiber = st.number_input("Fiber (ratio)", min_value=0.0, max_value=1.0, value=0.005, step=0.001, format="%.3f")

    with col2:
        cfa = st.number_input("Co-fired Fly Ash (ratio)", min_value=0.0, max_value=1.0, value=0.30, step=0.05)
        fa = st.number_input("Fly Ash (ratio)", min_value=0.0, max_value=1.0, value=0.00, step=0.05)
        sp = st.number_input("Superplasticizer (ratio)", min_value=0.0, max_value=1.0, value=0.02, step=0.01)

    with col3:
        rufa = st.number_input("Reactive Ultrafine FA (ratio)", min_value=0.0, max_value=1.0, value=0.00, step=0.01)
        agg = st.number_input("Fine Aggregate (ratio by total binders)", min_value=0.0, max_value=5.0, value=2.00, step=0.1)
    ...
    if st.button("Predict Compressive Strength", type="primary", use_container_width=True):
        ...
        if water <= 0.0001 or total_binder <= 0.0001:
            ...
            # Hitung rekayasa fitur pendukung
            ...
            # Rumus disesuaikan dengan Notebook asli Anda (WBR dibagi SP)
            sp_div_wbr = wbr / sp if sp > 0 else 0
            ...
            # Gabungkan semua data ke dalam kamus mentah
            raw_data = {
            ...
            # KUNCI UTAMA: Paksa urutan kolom DataFrame mengikuti cetakan scaler .pkl Anda secara mutlak
            kolom_wajib = list(main_scaler.feature_names_in_)
            input_df_final = pd.DataFrame([raw_data])[kolom_wajib]
            
            # Jalankan Transformasi Skala
            scaled_array = main_scaler.transform(input_df_final)
            
            # Kirim ke Core XGBoost menggunakan DMatrix
